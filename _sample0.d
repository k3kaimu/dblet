...
void main(){
    ...
    //式の書き方によらず、最終的な結果は同じになります(もちろん、計算結果が同じになるような簡約化しか行わない)
    static assert(is(typeof(ans) == typeof(C * beta + alpha * A * B)));
    static assert(is(typeof(ans) == typeof(C * beta + A * B * alpha)));
    static assert(is(typeof(ans) == typeof(C * beta + A * alpha * B)));
    
    /* TODO
     * ・構造体ベースからclassベースにすることにより、同一インスタンスではただ一度だけの演算にする。
     *  ・classで参照したほうが数学っぽい？
     *  ・参照だと混乱する？
     *  ・するにしても、classの崩壊的変更が入ってからの実装
     *
     * ・簡約化の手順をもっときっちりと定める。
     *  ・たとえば、2項演算子で、opBinaryとopBinaryRightがマッチした場合に、どちらを優先させるか
     *  　・優先順位を付ける？ blas level 1なら 優先順位1, blas level 3なら優先順位5とか
     *
     * ・特殊行列(対称,バンド,エルミート,etc...)
     *
     * ・LAPACKの充実化
     */
}